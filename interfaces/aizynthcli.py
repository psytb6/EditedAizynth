...
def _process_single_smiles(
    ...
    sts = finder.extract_statistics()
    if do_clustering:
        _do_clustering(
            finder,sts,detailed_resukts = False, model_path = route_distance_model
        )
    sts_str = "\n".join(
        f"{key.replace('_',' ')}: {value}" for key, value in sts.items()
    )
    f = open("temp_eUCT", "a+")
    f.write("\n")
    f.write(sts_str)
    f.write("\n\n")
   # f = open("stats_test.txt", "a+")  ####
   # f.write("\n\n")
   # f.write("eUCT2 Test \n")  ####
   # f.write(stats_str)  ####
   # f.write("\n") ####
...
def _process_multi_smiles(
    ...
    for smi in smiles:
        ...
        f = open("tests_UCT_unsolved_2000", "a+")
        f.write("\n")
        f.write(stats_str)
        f.write("\n\n")
        counter+=1
