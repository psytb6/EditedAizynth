...
class MctsSearchTree:
    ...
    def one_iteration(self) -> bool:
        ...
        if(leaf.state.score > self.config.current_best_score):
            self.config.current_best_score = leaf.state.score ####
            best_iteration = self.config.current_iteration   ####
            rollout_best = self.config.rollouts
            self.config.best_rollout = rollout_best
            self.config.iterations_best_sol = rollout_best  ####
