...
class MctsNode:
    ...
    def expand(self) -> None:
        ...
        npriors = len(self._children_priors)
       # self._config.branching_factors.append(nactions)
    ...
    def _children_eUCT(self) -> np.ndarray:
        total_visits = np.log(np.sum(self._children_visitations))
        child_visits = np.array(self._children_visitations)
        it_limit = self._config.iteration_limit  ####
        curr_it = self._config.current_iteration
        ts = (curr_it/it_limit)/2 ####
        q = self._config.C/(ts+1) ####
        return q * np.sqrt(2 * total_visits / child_visits) ####

    def _children_eUCT2(self) -> np.ndarray:
        total_visits = np.log(np.sum(self._children_visitations))
        child_visits = np.array(self._children_visitations)
        it_limit = self._config.iteration_limit  ####
        curr_it = self._config.current_iteration
