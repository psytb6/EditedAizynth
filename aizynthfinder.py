...
class AiZynthFinder:
    ...
    def extract_statistics(self) -> StrDict:
        ...
        stats = {
           ...
           # "branching_factors": self.config.branching_factors,
            "best_solution_rollout": self.config.iterations_best_sol,
            "total_rollouts": self.config.rollouts+ self.config.current_iteration,
        ...
        return stats
    def extract_mcts_statistics(self) -> StrDict:
        if not self.analysis:
            return {}
        stats = {
        "target": self.target_smiles,
        "search_time": self.search_stats["time"],
        "branching_factors": self.config.branching_factors,
        "best_solution_rollout": self.config.best_rollout+self.config.iterations_best_sol
        }
        stats.update(self.analysis.tree_statistics())
        return stats
    def prepare_tree(self) -> None:
        ...
        self.config.best_rollout = 0
        self.config.current_best_score = 0
        self.config.iterations_best_sol = 0
        self.config.current_iteration = 0
